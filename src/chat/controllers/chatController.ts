...
class ChatController {
   ...
   */
  createOrGetConversation = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      ...
      if (type === "direct") {
        if (!participantId) {
          throw new ApiError(
            400,
            "participantId requis pour les conversations directes"
          );
        }
  ...
  getUserConversations = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const userId = req.user.userId;
  ...
  sendMessage = asyncHandler(async (req: SendMessageRequest, res: Response) => {
    ...
    res
      .status(201)
      .json(new ApiResponse(201, message, "Message envoyé avec succès"));
  });
  /**
  ...
  reactToMessage = asyncHandler(async (req: ReactionRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, result, "Réaction ajoutée avec succès"));
  });
  ...
  deleteMessage = asyncHandler(async (req: DeleteRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, result, "Message supprimé avec succès"));
  });
  ...
  restoreMessage = asyncHandler(async (req: CustomRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, message, "Message restauré avec succès"));
  });
  ...
  searchMessages = asyncHandler(async (req: CustomRequest, res: Response) => {
    ...
    const rawMessageType = this.getStringFromQuery(req.query.messageType, "");
    const messageType: AllowedMessageType | undefined = allowedTypes.includes(
    ...
    const rawConversationId = this.getStringFromQuery(
  ...
  markConversationAsRead = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, {}, "Conversation marquée comme lue"));
    }
  );
  ...
  handleTyping = asyncHandler(async (req: CustomRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, {}, "Statut de frappe mis à jour"));
  });
  ...
  //   await this.verifyConversationAccess(conversationId, userId);
  //   const conversation = await this.chatService.archivedConversation(userId, conversationId);
  //   res.status(200).json(
  //     new ApiResponse(200, conversation, 'Conversation archivée avec succès')
  //   );
  // });
  archiveConversation = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const { conversationId } = req.params;
      const userId = req.user.userId;
      ...
      // Tu peux récupérer IP / userAgent ici si tu veux
      const ipAddress = req.ip || "unknown";
      const userAgent = req.get("User-Agent") || "unknown";

      const conversation = await this.chatService.archivedConversation(
        userId,
        conversationId,
        ipAddress,
        userAgent
  ...
  unarchiveConversation = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const { conversationId } = req.params;
      const userId = req.user.userId;
      ...
      const conversation = await this.chatService.unarchiveConversation(
        userId,
        conversationId,
        ipAddress,
        userAgent
      ...
      res
        ...
        .json(
          new ApiResponse(
            200,
            conversation,
            "Conversation déarchivée avec succès"
          )
        );
    }
  );
   ...
   */
  getConversationStats = asyncHandler(
    async (req: CustomRequest, res: Response) => {
      const { conversationId } = req.params;
      const userId = req.user.userId;
  ...
  async validateMessageByType(
    ...
    if (rule.requiredFile) {
      ...
      if (
        rule.mimetypePrefix &&
        !file.mimetype.startsWith(rule.mimetypePrefix)
      ) {
        throw new ApiError(
          400,
          `Le fichier doit être de type ${rule.mimetypePrefix}`
        );
      }

      if (
        rule.allowedMimetypes &&
        !rule.allowedMimetypes.includes(file.mimetype)
      ) {
      ...
      if (file.size > rule.maxSizeMB * 1024 * 1024) {
        throw new ApiError(
          400,
          `Fichier trop volumineux (max ${rule.maxSizeMB}MB)`
        );
      }
    }
  }
